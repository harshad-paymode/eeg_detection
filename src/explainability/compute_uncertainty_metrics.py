...
def compute_uncertainty_metrics():
    ...
    if OOD_DATA:
        target_names = sorted([p for p in os.listdir(TEST_DATA_DIR) if os.path.isdir(os.path.join(TEST_DATA_DIR, p))])
        # Dictionary to hold all passes across folds for OOD ensembling
        ood_ensemble_data = {t: {"all_preds": [], "ground_truth": None} for t in target_names}
    else:
        target_names = None

    # Dictionary to hold cross-fold metrics per target (only used for ID data now)
    summary_metrics = {}
    ...
    for n, fold in enumerate(fold_list):
        ...
        for t_name, t_dir, log_name in zip(current_targets, current_dirs, log_names):
            ...
            # =========================================================
            # OOD DATA ROUTINE (Accumulate across folds)
            # =========================================================
            if OOD_DATA:
                ood_ensemble_data[t_name]["all_preds"].extend(all_preds)
                if ood_ensemble_data[t_name]["ground_truth"] is None:
                    ood_ensemble_data[t_name]["ground_truth"] = ground_truth
                
                # Skip per-fold calculations for OOD
                continue 

            # =========================================================
            # ID DATA ROUTINE (Per-fold calculation - Untouched)
            # =========================================================
            wandb.init(project=project_name, name=log_name, config=INITIAL_CONFIG)
            ece_metric = MulticlassCalibrationError(num_classes=3, n_bins=15, norm='l1').to(device)
            
            # Aggregate purely within fold
            if INITIAL_CONFIG['mc_dropout']:
                preds_raw = torch.stack(all_preds).mean(dim=0)
                preds = preds_raw.argmax(dim=1)
            else:
            ...
            # Calculating predictions and correct/incorrect flag
            preds_class = preds_raw.argmax(dim=1)
            is_correct = (preds_class == ground_truth)
    ...
    # =============================================================
    # OOD DATA ENSEMBLE EVALUATION & LOGGING (Post-fold loop)
    # =============================================================
    if OOD_DATA:
        for t_name, data in ood_ensemble_data.items():
            print(f"Computing ensembled metrics for OOD target: {t_name}")
            
            all_preds = data["all_preds"]
            ground_truth = data["ground_truth"].to(device)

            # Aggregate across all folds and all passes (num_folds * num_passes)
            preds_raw = torch.stack(all_preds).mean(dim=0).to(device)
            preds_class = preds_raw.argmax(dim=1)
            ...
            # WandB logging for ensembled results
            ...
            # Save ensembled scalar metrics dictionary
            fold_results = {
                "fold": "ensemble", 
                "target": t_name,
                "ECE": fold_ece,
            ...
            ensemble_res_path = os.path.join(SAVE_DIR_METRICS, f"{t_name}_ensemble_unc_results.json")
            with open(ensemble_res_path, "w") as f:
                json.dump(fold_results, f)

            is_correct = (preds_class == ground_truth)

            # Save ensembled per-sample metrics array
            per_sample_data = {
                "sample_id": np.array([f"ensemble_{t_name}_{i}" for i in range(len(ground_truth))]), # Ensemble + Patient + Index
                "mean_probs": preds_raw.cpu().numpy(),
                "pred_labels": preds_class.cpu().numpy(),
                "true_labels": ground_truth.cpu().numpy(),
                "confidence": preds_raw.max(dim=1).values.cpu().numpy(),
            ...
            ensemble_npz_path = os.path.join(SAVE_DIR_METRICS, f"{t_name}_ensemble_per_sample_data.npz")
            np.savez(ensemble_npz_path, **per_sample_data)

    # =============================================================
    # ID DATA FINAL SUMMARY LOGGING (Post-fold loop)
    # =============================================================
    elif not OOD_DATA:
        for t_name, metrics in summary_metrics.items():
            log_name = "summary_uncertainty"
            wandb.init(project=project_name, name=log_name)
