...
def compute_uncertainty_metrics():
    ...
    # =========================================================================
    # --- NEW ADDITION: Setup variable to track passes for Ensemble ---
    ensemble_tracking = {}
    if OOD_DATA:
    ...
    for n, fold in enumerate(fold_list):
        ...
        for t_name, t_dir, log_name in zip(current_targets, current_dirs, log_names):
            ...
            # =========================================================================
            # --- NEW ADDITION: Store passes and ground truth for Ensemble ---
            if OOD_DATA:
                # Store passes on CPU to save memory across 10 folds
                if INITIAL_CONFIG['mc_dropout']:
                    ensemble_tracking[t_name]["all_passes"].extend([p.detach().cpu() for p in all_preds])
                else:
                    ensemble_tracking[t_name]["all_passes"].append(torch.nn.functional.softmax(all_preds[0], dim=1).detach().cpu())
    ...
    if OOD_DATA:
        ...
        for t_name, data in ensemble_tracking.items():
            ...
            # Stack and average probabilities across all pooled passes (10 for base, 500 for MC)
            ...
            # Calculate general uncertainty metrics
            ...
            ens_results = {
                ...
                "NLL": ens_nll,
                "AURC": ens_aurc,
                "Total_Entropy": ens_total_entropy,
                "Aleatoric_Entropy": ens_aleatoric_entropy,
            ...
            # Save per-sample data for plotting AUROC and Density later
